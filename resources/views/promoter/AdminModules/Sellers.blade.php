        ...
        <div class="col s4">
          <div class="card blue-grey darken-1">
            <div class="card-content white-text">
              <span class="card-title">ID: {{$modules->id}} || Nombre: {{$modules->name}}</span>
            </div>
          </div>
        </div>
    @endforeach
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      $.each(data,function(i,info) {
        ...
        if (info.logo!=null) {
          var logo = "<img class='materialboxed' width='100' height='90' src='{!!asset('"+info.logo+"')!!}'";
        } else {
