...
class AssingPoints
{
    ...
    {
      $TicketsPackage = TicketsPackage::find($event->package_id); // busca el paquete
      $User = User::find($event->user_id); // busca el usuario que esta haciendo la compra
      $points= $TicketsPackage->points; // los puntos que da al comprar el paquete 
      $i = 0; // no sirve esta variable
      if ($User->UserRefered->count() == 0) { // si el usuario no tiene patrocinador
        ...
        foreach ($Refered as $key) {
          if($key->points == $key->limit_points) {
            // si el usuario llega a su limite de puntos, los puntos pasan a Leipel
