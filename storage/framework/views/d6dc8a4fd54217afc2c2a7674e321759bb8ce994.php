...
<?php $__env->startSection('js'); ?>
	...
	<script>
		function musicos(status) {
            ...
            $.ajax({
                url: "<?php echo url('MusicianData'); ?>/"+status,
                type:'GET',
                dataType: "json",
                success: function (data) {
                    ...
                    $.each(data,function(i,info) {
                    	if (info.photo!=null) {
                    		var foto = 
                    		"<img class='materialboxed' src='<?php echo asset('"+info.photo+"'); ?>' style='width:70px;height:70px;' alt=Foto>";
                    	} else {
	    ...
	    $(document).on('click', '#Status', function() {
	    	...
	    	$("#formStatus").on('submit', function(e) {
	    	 	...
	    	 	var url = "<?php echo e(url('admin_musician/')); ?>/"+x;
        ...
        $(document).on('click', '#rejectAuthorMusic', function(e) {
            ...
            var url = "<?php echo url('viewRejection/"+idMusico+"/"+modulo+"'); ?>";
            console.log(url);
            $("#negaciones").empty();
            e.preventDefault();
            $.ajax({
