...
<?php echo $__env->make('promoter.modals.MoviesViewModal', array_except(get_defined_vars(), array('__data', '__path')))->render(); ?>
	...
	$(document).ready(function(){
		...
		$(document).on('click','#status', function() {
			...
			$("#formStatus").on('submit', function(e) {
	            ...
	            var url = "<?php echo e(url('/admin_movie/')); ?>/"+idMovie;
		...
		$(document).on('click','#viewMovie', function() {
			...
			var idMovie = $(this).val();
			console.log(idMovie);
			var url = "<?php echo e(url('/viewMovie/')); ?>/"+idMovie;
			$.ajax({
				url: url,
				type: 'get',
				dataType: 'json', 
				success: function (result) {
					console.log(result);
					var rutaPelicula = "<?php echo e(asset('/')); ?>movie/film/"+result.duration;
					var rutaPoster = "<?php echo e(asset('/')); ?>movie/poster/"+result.img_poster;
		...
		var Movies = $('#peliculas').DataTable({
	        ...
	        ajax: '<?php echo url('MoviesDataTable/En Proceso'); ?>',
	        columns: [
		...
		$(document).on('click','#opcion2', function() {
			...
			var peliculasAprobadas = $('#peliculas').DataTable({
		        ...
		        ajax: '<?php echo url('MoviesDataTable/Aprobado'); ?>',
		        columns: [
		        	{data: 'autor', name: 'autor'},
		            {data: 'img_poster', name: 'img_poster'},
		            {data: 'title', name: 'title'},
		...
		$(document).on('click','#opcion3', function() {
			...
			var peliculasRechazadas = $('#peliculas').DataTable({
		        ...
		        ajax: '<?php echo url('MoviesDataTable/Denegado'); ?>',
		        columns: [
		        	{data: 'autor', name: 'autor'},
		            {data: 'img_poster', name: 'img_poster'},
		            {data: 'title', name: 'title'},
