...
<script id="jsbin-javascript">
  ...
  function setClient(status) {
    ...
    var ruta = "";
    if (status==0) {
      ruta = "<?php echo e(url('ClientsDataTable')); ?>";
    } else if (status==1) {
      ruta = "<?php echo e(url('AllClientsDataTable')); ?>";
    } else if (status==2) {
      ruta = "<?php echo e(url('RejectedClientsDataTable')); ?>";
    }
      ...
      success: function (data) {
        ...
        $.each(data,function(i,info) {
          ...
          if (info.img_doc!=null) {
            var doc = "<img class='materialboxed' width='100' height='90' src='<?php echo asset('"+info.img_doc+"'); ?>'";
          } else {
  ...
  $(document).ready(function(){
    ...
    $(document).on('click', '#depositos', function() {
      $("#pagos").empty();
      var ruta ="<?php echo e(url('DepsitDataTable')); ?>";
      var gif = "<?php echo e(asset('/sistem_images/loading.gif')); ?>";
      swal({
        title: "Procesando la información",
        text: "Espere mientras se procesa la información.",
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          $.each(data,function(i,info) {
            ...
            if (info.voucher!=0) {
              var voucher = "<img class='materialboxed' width='100' height='90' src='<?php echo asset('"+info.voucher+"'); ?>'";
            } else {
    ...
    $(document).on('click', '#user', function() {
      ...
      console.log(id);
      var url = "<?php echo e(url('/infoUser/')); ?>/"+id;
      $.ajax({
        url: url,
        type: 'get',
        dataType: 'json', 
        success: function (info) {
          ...
          if (info.img_perf!=null) {  
            var perfilUsuario = "<?php echo e(asset('/')); ?>/"+info.img_perf;
            $("#perfilUsuario").attr('src',perfilUsuario);
          } else {
            $("#perfilUsuario").attr('src',"<?php echo e(asset('plugins/img/sinPerfil.png')); ?>");
          }
          ...
          if (info.img_doc!=null) { 
            var imgRucUsuario = "<?php echo e(asset('/')); ?>/"+info.img_doc;
            $("#imgRucUsuario").show();
    ...
    $(document).on('click', '#Status', function() {
      ...
      $( "#formStatus" ).on( 'submit', function(e) {
        ...
        var url = "<?php echo e(url('ValidateUser/')); ?>/"+x;
        console.log(message);
        console.log(s);
        e.preventDefault();
        var gif = "<?php echo e(asset('/sistem_images/loading.gif')); ?>";
    ...
    $(document).on('click', '#webs', function() {
      $("#redes").empty();
      var x = $(this).attr("value");
      var url = "<?php echo e(url('/ReferalsDataTable/')); ?>/"+x;
      $.ajax({
        url: url,
        type: 'get',
        dataType: 'json', 
    ...
    $(document).on('click', '#rejectPayments', function(e) {
      ...
      console.log(idUser);
      var modulo = "User";
      var url = "<?php echo url('viewRejection/"+idUser+"/"+modulo+"'); ?>";
      console.log(url);
      $("#negaciones").empty();
      e.preventDefault();
      $.ajax({
